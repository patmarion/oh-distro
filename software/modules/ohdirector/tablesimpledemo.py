...
#        self.ikPlanner.ikServer.removeAffordanceFromLink( self.ikPlanner.getHandLink(side) , obj.getProperty('Name'))
    ...
    #mfdef waitForCleanLidarSweepAsync(self):
    #mf    currentRevolution = self.multisenseDriver.displayedRevolution
    #mf    desiredRevolution = currentRevolution + 2
    #mf    while self.multisenseDriver.displayedRevolution < desiredRevolution:
    #mf        yield
...
class TableSimpleTaskPanel(TaskUserPanel):
    ...
    def addButtons(self):
        ...
        self.addManualButton('Lower arm', functools.partial(self.tableSimpleDemo.planLowerArm, self.tableSimpleDemo.graspingHand))
        self.addManualSpacer()
        self.addManualButton('Raise arm', self.tableSimpleDemo.planPreGrasp)
        self.addManualSpacer()
        self.addManualButton('Commit Manip', self.tableSimpleDemo.commitManipPlan)
        self.addManualSpacer()
        self.addManualButton('Open Hand', functools.partial(self.tableSimpleDemo.openHand, self.tableSimpleDemo.graspingHand))
        self.addManualSpacer()
        self.addManualButton('Close Hand', functools.partial(self.tableSimpleDemo.closeHand, self.tableSimpleDemo.graspingHand))
        self.addManualSpacer()
        self.addManualButton('Read SDF & Sim', self.tableSimpleDemo.loadSDFFileAndRunSim)
    ...
    def addDefaultProperties(self):
        ...
        if self.tableSimpleDemo.ikPlanner.fixedBaseArm:
        ...
        if not self.tableSimpleDemo.ikPlanner.fixedBaseArm:
        ...
        self.tableSimpleDemo.graspingHand = self.getSide()
        self.tableSimpleDemo.lockBase = self.getLockBase()
        self.tableSimpleDemo.lockBack = self.getLockBack()
        self.tableSimpleDemo.sceneID = self.getSceneId()
        self.tableSimpleDemo.sceneName = self.getSceneName()
        self.tableSimpleDemo.planner = self.getPlanner()
        if 'userConfig' in drcargs.getDirectorConfig() and 'useKuka' in drcargs.getDirectorConfig()['userConfig']:
    ...
    def onPropertyChanged(self, propertySet, propertyName):
        ...
        if propertyName == 'Hand':
            ...
            if self.handEngaged: # was engaged, hence deactivate
                self.tableSimpleDemo.getHandDriver(self.getSide()).sendDeactivate() # deactivate hand
            else: # was disenaged, hence activate
                self.tableSimpleDemo.getHandDriver(self.getSide()).sendActivate() # activate hand
            self.handEngaged = self.getHandEngaged()

        elif propertyName == 'Scene':
            self.tableSimpleDemo.sceneID = self.params.getProperty('Scene')
            self.tableSimpleDemo.sceneName = self.params.getPropertyEnumValue('Scene')
		
        elif propertyName == 'Planner':
            self.tableSimpleDemo.planner = self.params.getPropertyEnumValue('Planner')
        self.syncIkPlannerOptions()
    ...
    def addTasks(self):
        ...
        def addManipulation(func, name, parent=None, confirm=False):
            ...
            if self.tableSimpleDemo.planner != 1:
        ...
        #    # mfallon addFunc(v.searchFinalPose, 'search final pose', parent=group)
...
#            addFunc(v.planWithdrawTableObject, 'withdraw object', parent='lift object', confirm=False)
        ...
        if not v.ikPlanner.fixedBaseArm:
            ...
            addFunc(self.tableSimpleDemo.moveRobotToBinStanceFrame, 'walk to Bin', parent = walkToBin)

...
joints2frames::joints2frames(std::shared_ptr<lcm::LCM>& lcm_, bool show_labels_,
                             bool show_triads_)
    : lcm_(lcm_), show_labels_(show_labels_), show_triads_(show_triads_) {

  botparam_ = bot_param_new_from_server(lcm_->getUnderlyingLCM(), 0);

  model_ = std::shared_ptr<ModelClient>(
      new ModelClient(lcm_->getUnderlyingLCM(), 0));
  KDL::Tree tree;
  if (!kdl_parser::treeFromString(model_->getURDFString(), tree)) {
    cerr << "ERROR: Failed to extract kdl tree from xml robot description"
         << endl;
    exit(-1);
  }
  fksolver_ = std::shared_ptr<KDL::TreeFkSolverPosFull_recursive>(
      new KDL::TreeFkSolverPosFull_recursive(tree));
  ...
  pub_frequency_["BODY_TO_BUMBLEBEE2"] = FrequencyLimit(0, 1E6 / 100);
#endif
}
...
void joints2frames::publishPose(Eigen::Isometry3d pose, int64_t utime,
                                std::string channel) {
  if (!shouldPublish(utime, channel)) return;
...
void joints2frames::publishRigidTransform(Eigen::Isometry3d pose, int64_t utime,
                                          std::string channel) {
  if (!shouldPublish(utime, channel)) return;
...
bool joints2frames::shouldPublish(int64_t utime, std::string channel) {
  ...
  if (it == pub_frequency_.end()) {
    std::cout << channel
              << " was not found in the pub_frequency list - not publishing\n";
    return false;
  }

  if (utime < it->second.last_utime) {
    it->second.last_utime = 0;
    std::cout << utime << " detected negative time change for " << channel
              << " resetting last_utime\n";
  }
...
void joints2frames::robot_state_handler(const lcm::ReceiveBuffer* rbuf,
                                        const std::string& channel,
                                        const bot_core::robot_state_t* msg) {
  // 0. Extract World Pose of body:
  Eigen::Isometry3d world_to_body;
  world_to_body.setIdentity();
  world_to_body.translation() << msg->pose.translation.x,
      msg->pose.translation.y, msg->pose.translation.z;
  ...
  for (uint i = 0; i < (uint)msg->num_joints;
       i++)  // cast to uint to suppress compiler warning
    jointpos_in.insert(make_pair(msg->joint_name[i], msg->joint_position[i]));

  // Calculate forward position kinematics
  bool kinematics_status;
  bool flatten_tree = true;  // determines absolute transforms to robot origin,
                             // otherwise relative transforms between joints.
  kinematics_status =
      fksolver_->JntToCart(jointpos_in, cartpos_out, flatten_tree);
...
cout << "kin status " << kinematics_status << endl;
std::cout << "cartpos_out size" << cartpos_out.size() << std::endl;

  // 2. Determine the required BOT_FRAMES transforms:
  // for (std::map<std::string, KDL::Frame>::iterator ii = cartpos_out.begin();
  //      ii != cartpos_out.end(); ++ii) {
  for (auto& ii : cartpos_out) {
    cout << ii.first << "; " << endl; //<< ii.second << endl;
    // cout << " a : ";
    // cout << "check " << (*ii).first << std::endl;
    // if ((*ii).first.compare("bumblebee2") == 0) {
    //   cout << "Publishing transform" << std::endl;
    //   Eigen::Isometry3d body_to_head = KDLToEigen((*ii).second);
    //   publishRigidTransform(body_to_head, msg->utime, "BODY_TO_BUMBLEBEE2");
    // }
  }
  ...
  if (show_triads_) {
    ...
    for (map<string, KDL::Frame>::iterator ii = cartpos_out.begin();
         ii != cartpos_out.end(); ++ii) {
    ...
    pc_vis_->pose_collection_to_lcm_from_list(
        6001, world_to_jointsT);  // all joints in world frame
    if (show_labels_)
      pc_vis_->text_collection_to_lcm(6002, 6001, "Frames [Labels]",
                                      joint_names, body_to_joint_utimes);
  }
}
